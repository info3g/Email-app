...
function bp_friends_random_friends() {
	...
	<div class="info-group">
		<h4>
			...
			bp_word_or_name( __( "My Subscriptions", 'buddypress' ), __( "%s's Subscriptions", 'buddypress' ) );
			?>
			&nbsp;
		...
		<?php } else { ?>
			...
			<div id="message" class="info">
				<p>
					<?php
					/* translators: %s: member name */
					bp_word_or_name( __( "You haven't added any Subscription connections yet.", 'buddypress' ), __( "%s hasn't created any friend connections yet.", 'buddypress' ) );
					?>
				</p>
			</div>
...
function bp_friend_search_form() {
	...
	$label  = __( 'Filter Subscriptions', 'buddypress' ); ?>
...
function bp_member_total_friend_count() {
	...
	function bp_get_member_total_friend_count() {
		...
		return apply_filters(
			...
			sprintf( _n( '%d Subscription', '%d Subscriptions', $total_friend_count, 'buddypress' ), number_format_i18n( $total_friend_count ) )
		);
	}
		...
		switch ( $is_friend ) {
			case 'pending':
				$button = array(
					...
					'link_text'         => __( 'Cancel Subscriptions Request', 'buddypress' ),
			...
			case 'is_friend':
				...
				$button = array(
					...
					'link_text'         => __( 'Cancel Subscriptions', 'buddypress' ),
					'link_id'           => 'friend-' . $potential_friend_id,
					'link_rel'          => 'remove',
			...
			default:
				$button = array(
					...
					'link_text'         => __( 'Add Subscriptions', 'buddypress' ),
					'link_id'           => 'friend-' . $potential_friend_id,
					'link_rel'          => 'add',
...
function bp_friends_get_profile_stats( $args = '' ) {
	...
	if ( empty( $r['output'] ) ) {
		...
		if ( ! empty( $r['user_id'] ) ) {
			...
			/* translators: %d: total friend count */
			$r['output'] .= sprintf( _n( '%d Subscription', '%d Subscriptions', $r['friends'], 'buddypress' ), '<strong>' . number_format_i18n( $r['friends'] ) . '</strong>' );
			$r['output'] .= $r['after'];
		}
	}
