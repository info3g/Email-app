...
$where = get_posts_by_author_sql('post', TRUE, $theuser);
$count = $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->posts $where" );
  
 
      bp_core_new_nav_item( array( 
            'name' => 'Works <span class="count">'.$count.'</span>', 
            'slug' => 'Works', 
            'screen_function' => 'yourtab_screen', 
            'position' => 40,
            'parent_url'      => bp_loggedin_user_domain() . '/works/',
      ...
      while($query->have_posts()) :
       ...
       $content = get_the_content();
        ...
        $cats = get_the_category($id);
           ?>
<article id="post-250" class="blog-entry clr no-featured-image large-entry post-250 post type-post status-publish format-standard hentry category-writing language-english entry">
...
<ul class="meta obem-none clr" aria-label="Post details:">
					...
					<li class="meta-cat"><span class="screen-reader-text">Post category:</span><i class=" icon-folder" aria-hidden="true" role="img"></i><a href="https://growadandelion.ca/category/<?php echo $cats[0]->cat_slug; ?>/" rel="category tag"><?php echo $cats[0]->cat_name; ?></a></li>
		
		
					<li class="meta-author" itemprop="name"><span class="screen-reader-text">Post author:</span><i class=" icon-user" aria-hidden="true" role="img"></i><a href="https://growadandelion.ca/members/<?php echo get_the_author(); ?>/" title="dandelion.nmt" rel="author" itemprop="author" itemscope="itemscope" itemtype="https://schema.org/Person"><?php echo get_the_author(); ?></a></li>
	
		
					<li class="meta-date" itemprop="datePublished"><span class="screen-reader-text">Post published:</span><i class=" icon-clock" aria-hidden="true" role="img"></i><?php echo get_the_date(); ?></li>
		

		
					<li class="meta-mod-date" itemprop="dateModified"><span class="screen-reader-text">Post last modified:</span><i class=" icon-note" aria-hidden="true" role="img"></i><?php echo get_the_modified_date(); ?></li>
		
					<li class="meta-comments"><span class="screen-reader-text">Post comments:</span><i class=" icon-bubble" aria-hidden="true" role="img"></i><a href="<?php echo get_comment_link(); ?>" class="comments-link"><?php echo get_comments_number($id); ?> Comment</a></li>
...
<div class="blog-entry-summary clr" itemprop="text">
		<p><?php echo get_the_excerpt();//wp_trim_words( $content, 9, NULL ); ?></p>		
</div><!-- .blog-entry-summary -->



<div class="blog-entry-readmore clr">
	<a href="<?php echo get_the_permalink(); ?>">Continue Reading<span class="screen-reader-text"><?php echo get_the_title(); ?></span><i class=" fa fa-angle-right" aria-hidden="true" role="img"></i></a>
...
global $bp;
$theuser = bp_displayed_user_id();
  $cc = get_user_meta($theuser, 'total_friend_count');
// Change name of menu item
$bp->bp_nav['activity']['name'] = 'Activities';
$bp->bp_nav['friends']['name'] = 'Subscriptions <span class="count">'.$cc[0].'</span>';
...
$button['link_title'] = 'Cancel Subscription';
$button['link_class'] .= ' add-friend';
break;

case 'is_friend' :
$button['link_text'] = $fricon_remove.__(' Remove Subscription!', 'domain');;
$button['link_class'] .= ' add-friend';
break;

default:
$button['link_text'] = $fricon_add.__('Subscribe to', 'domain');
$button['link_title'] = 'Subscribe to';
$button['link_class'] .= ' add-friend';
}
return $button;
}
...
function dc_bp_filter_author_link( $link, $author_id ) {
	...
	$link = 'https://growadandelion.ca/members/' . $username. '/Works';
return $link;
  //return bp_core_get_user_domain( $author_id );
}

/* add update data hook in wp friends */
...
function my_custom_notify_frnd($post_ID, $post, $update){
  //global $bp;
  global $wpdb;
// Bail if activity is not active
   ...
   if( $seconds_difference <= 2 ){
   	 if ( !wp_is_post_autosave( $post ) && $post->post_status !== 'auto-draft' && 
         !wp_is_post_revision( $post_ID ) ) {
   	//if ( $post->post_status == 'publish' ){
  //if(get_post_status($post_ID) == 'publish'){
        
          $useridd = get_current_user_id();
           $slug = basename(get_permalink($post_ID));
          // select   
           $postNAME = get_the_title($post_ID);
          $user_info = get_userdata($useridd);
         
           $results = $wpdb->get_results( "SELECT * FROM gzn_bp_friends where friend_user_id=$useridd");
           if(!empty($results)){ 
           //print_r($results);
           	...
           	foreach($results as $row){   
           		...
           		if($row->initiator_user_id != $useridd){
           		///	echo $row->initiator_user_id;
               $username = $user_info->user_nicename;
   
          $newfrnduserid2 = $row->initiator_user_id;
           $chk_emailsetting2 = get_user_meta($newfrnduserid2, 'notification_friends_new_custom');
           if($chk_emailsetting2[0] == 'yes'){ 
          $user_info2 = get_userdata($newfrnduserid2);
$username2 = $user_info2->display_name;
            $usereml2 = $user_info2->user_email;
             $msg2 = '<table cellpadding="0" cellspacing="0" border="0" height="100%" width="100%" style="border-collapse:collapse;" class="email_bg"><tr><td valign="top">
	<center style="width: 100%; text-align: left>;">
		...
		<div style="max-width: 600px; margin: auto;" class="email-container">
			...
			<table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" width="100%" style="max-width:600px;border-top:7px solid #58962b" bgcolor="#F7F3F0">
				<tbody><tr>
					<td style="text-align:center;padding:15px 0;font-family:sans-serif;font-weight:bold;color:#555555;font-size:30px">
						Grow a Dandelion	</td>
				</tr>
			</tbody></table>
		
			<table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" bgcolor="#FFFFFF" width="100%" style="max-width:600px;border-radius:5px">
						...
						<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
							<tbody><tr>
								<td style="padding:20px;font-family:sans-serif;line-height:24px;color:#555555;font-size:15px">
									<span style="font-weight:bold;font-size:20px">Hi '.$username2.',</span>
									<hr color="#F7F3F0">
									<p>'.$username. ' published a new post.</p>
									<p>"'.$postNAME. '"</p>
<p><a style="color:#58962b" href="https://growadandelion.ca/'.$username.'/'.$slug.'/" target="_blank" >Go to the discussion</a> to reply or catch up on the conversation.</p>
			...
			<table role="presentation" cellspacing="0" cellpadding="0" border="0" align="left" width="100%" style="max-width:600px;border-radius:5px" bgcolor="#F7F3F0">
				<tbody><tr>
					<td style="padding:20px;width:100%;font-size:12px;font-family:sans-serif;line-height:19px;text-align:left;color:#555555;word-break:break-all">
						
						<span>© 2022 Grow a Dandelion</span>
           ...
           $subjectvalue = get_field( "email_subject_text", 373 );
      if($subjectvalue){

      	$subject = str_replace("{{postAuthorName}}", $username, $subjectvalue);
      	
      }else{
      	$subject = "[Grow a Dandelion] Your Subscriber $username published a new post!";
      }
          
  
          wp_mail($usereml2, $subject, $msg2);
}
}
...
<p><a style="color:#58962b" href="https://growadandelion.ca/'.$username.'/'.$slug.'/" target="_blank" >Go to the discussion</a> to reply or catch up on the conversation.</p>
          ...
          $subjectvalue = get_field( "email_subject_text", 375 );
      if($subjectvalue){

      	$subject = str_replace("{{postAuthorName}}", $username, $subjectvalue);
      	
      }else{
      	$subject = "[Grow a Dandelion] Your Subscriber $username updated a post!";
      }
...
add_action('save_post_post','my_custom_notify_frnd', 10, 3 );


/* send emails when someone comment on post of author */
function wpdocs_notify_my_mail( $comment_id, $comment_approved ) {
       global $wpdb;
       $comment = get_comment( $comment_id );
	   $comment_postid = $comment->comment_post_ID;
		
		  $results = $wpdb->get_results( "SELECT post_author FROM gzn_posts where ID=$comment_postid");
           if(!empty($results)){ 
           //print_r($results);
		...
		$post_auth_id = $row->post_author;
		 $chk_emailsetting1 = get_user_meta($post_auth_id, 'notification_friends_new_post_comment');
		 if($chk_emailsetting1[0] == 'yes'){
		$user_info = get_userdata($post_auth_id);
        $mail = $user_info->user_email;
		$ulogin = $user_info->user_login;
		$display_name = $user_info->display_name;
		$slug = basename(get_permalink($comment_postid));
		$commentlink = 'https://growadandelion.ca/'.$ulogin.'/'.$slug.'/#comment-'.$comment_id;
		$subjectvalue = get_field( "email_subject_text", 388 );
      if($subjectvalue){

      	$subject = str_replace("{{comment_author}}", $comment->comment_author, $subjectvalue);
      	
      }else{
     $subject = "[Grow a Dandelion] $comment->comment_author Commented on your post";
      }
       
        $message = $comment->comment_content;
		
		$msg = '<table cellpadding="0" cellspacing="0" border="0" height="100%" width="100%" style="border-collapse:collapse;" class="email_bg"><tr><td valign="top">
	<center style="width: 100%; text-align: left>;">
				...
				<tbody><tr>
					<td>
						<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
							<tbody><tr>
								<td style="padding:20px;font-family:sans-serif;line-height:24px;color:#555555;font-size:15px">
									<span style="font-weight:bold;font-size:20px">Hi '.$display_name .',</span>
									<hr color="#F7F3F0">
									<p>'.$comment->comment_author. ' Commented on your post.</p>
									<p>"'.$message. '"</p>
<p><a style="color:#58962b" href="'.$commentlink.'" target="_blank" >Go to the discussion</a> to reply or catch up on the conversation.</p>
...
/* send emails when author replied on your comment */
function wpdocs_notify_my_mailCommentRep( $comment_id, $comment_approved ) {

       $comment = get_comment( $comment_id );
	   $comment_postid = $comment->comment_post_ID;
	    $comment_isa_reply_of_commentid = $comment->comment_parent;
		         if($comment_isa_reply_of_commentid != '0'){
					 $comment2 = get_comment( $comment_isa_reply_of_commentid );
					 // now email of the comment author who got reply
					$mail = $comment2->comment_author_email;
		 
        // now user id of getting reply author
           			$get_reply_userid = $comment2->user_id;
		
		 $chk_emailsetting1 = get_user_meta($get_reply_userid, 'notification_friends_new_post_getcomment_reply');
		 if($chk_emailsetting1[0] == 'yes'){
		$user_info = get_userdata($get_reply_userid);
        
		$ulogin = $user_info->user_login;
		$display_name = $user_info->display_name;
		$slug = basename(get_permalink($comment_postid));
		$commentlink = 'https://growadandelion.ca/'.$ulogin.'/'.$slug.'/#comment-'.$comment_id;
		$subjectvalue = get_field( "email_subject_text", 391 );
      if($subjectvalue){

      	$subject = str_replace("{{postAuthorName}}", $comment->comment_author, $subjectvalue);
      	
      }else{
      $subject = "[Grow a Dandelion] $comment->comment_author replied on your comment";
      }
						...
						<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
							<tbody><tr>
								<td style="padding:20px;font-family:sans-serif;line-height:24px;color:#555555;font-size:15px">
									<span style="font-weight:bold;font-size:20px">Hi '.$display_name .',</span>
									<hr color="#F7F3F0">
									<p>'.$comment->comment_author. ' replied on your comment.</p>
									<p>"'.$message. '"</p>
<p><a style="color:#58962b" href="'.$commentlink.'" target="_blank" >Go to the discussion</a> to reply or catch up on the conversation.</p>
...
add_action( 'comment_post', 'wpdocs_notify_my_mailCommentRep', 10, 2 );
...
function wp_set_email_notifications_preference( $user_id ) {
  $useridd = get_current_user_id();
    ...
    $settings_keys = array(
        'notification_friends_new_custom' => 'yes',
         'notification_friends_new_post_comment' => 'yes',
         'notification_friends_new_post_getcomment_reply' => 'yes'
        
    );
 
    foreach( $settings_keys as $setting => $preference ) {
 
        bp_update_user_meta( $useridd,  $setting, $preference );
    }

}


/* disable plugin auto up*/
...
// Set UI labels for Custom Post Type
    $labels = array(
        'name'                => _x( 'Custom Emails Text', 'Post Type General Name', 'oceanwp' ),
        'singular_name'       => _x( 'Custom Email Text', 'Post Type Singular Name', 'oceanwp' ),
        'menu_name'           => __( 'Custom Emails Text', 'oceanwp' ),
        'parent_item_colon'   => __( 'Parent Custom Emails Text', 'oceanwp' ),
        'all_items'           => __( 'All Custom Emails', 'oceanwp' ),
        'view_item'           => __( 'View Custom Emails', 'oceanwp' ),
        'add_new_item'        => __( 'Add New Custom Email', 'oceanwp' ),
        'add_new'             => __( 'Add New', 'oceanwp' ),
        'edit_item'           => __( 'Edit Custom Emails', 'oceanwp' ),
        'update_item'         => __( 'Update Custom Emails', 'oceanwp' ),
        'search_items'        => __( 'Search Custom Emails', 'oceanwp' ),
...
// Set other options for Custom Post Type
    ...
    $args = array(
        'label'               => __( 'Custom Emails Text', 'oceanwp' ),
        'description'         => __( 'Custom Emails Text', 'oceanwp' ),
        'labels'              => $labels,
    ...
    register_post_type( 'custom_emails_text', $args );
...
function buddydev_hide_add_friend_button( $button ) {
	...
	$useriddd = get_current_user_id();
	if($displayed_user_id != $useriddd){
     $results = $wpdb->get_results( "SELECT * FROM gzn_bp_friends where initiator_user_id=$useriddd and friend_user_id=$displayed_user_id");
           if(empty($results)){ 
           //	if($displayed_user_id == $useriddd){ 

                $us_id = bp_get_member_user_id();
                $button = array(
					'id'                => 'not_friends',
					'component'         => 'friends',
					...
					'wrapper_id'        => 'friendship-button-' . $displayed_user_id,
					'link_href'         => wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/add-friend/' . $displayed_user_id . '/', 'friends_add_friend' ),
					'link_text'         => __( 'Add Subscription', 'buddypress' ),
					'link_id'           => 'friend-' . $displayed_user_id,
					'link_rel'          => 'add',
...
$output .='<ul class="wp-block-latest-posts__list has-dates wp-block-latest-posts">
<li><a href="'.get_the_permalink().'">'.get_the_title().'</a><br>
'.get_the_date().'</li>
</ul>';
	...
	add_shortcode( 'blogs-list', 'diwp_create_shortcode_movies_post_type' ); 
