	...
	switch ( $action ) {
		case 'friendship_accepted':
			...
			if ( (int) $total_items > 1 ) {
				...
				$text = sprintf( __( '%d has subscribed to you', 'buddypress' ), (int) $total_items );
				$amount = 'multiple';
			} else {
				/* translators: %s: friend name */
				$text = sprintf( __( '%s has subscribed to you', 'buddypress' ), bp_core_get_user_displayname( $item_id ) );
				$amount = 'single';
			}
...
function friends_screen_notification_settings() {
	...
	if ( ! $accept_requests = bp_get_user_meta( bp_displayed_user_id(), 'notification_friends_friendship_accepted', true ) )
		$accept_requests = 'yes'; ?>
  <?php
	if ( ! $accept_requestss = bp_get_user_meta( bp_displayed_user_id(), 'notification_friends_new_custom', true ) )
		$accept_requestss = 'yes'; ?>

 <?php
	if ( ! $accept_requestss2 = bp_get_user_meta( bp_displayed_user_id(), 'notification_friends_new_post_comment', true ) )
		$accept_requestss2 = 'yes'; ?>


<?php
	if ( ! $accept_requestss23 = bp_get_user_meta( bp_displayed_user_id(), 'notification_friends_new_post_getcomment_reply', true ) )
		$accept_requestss23 = 'yes'; ?>
	...
	<table class="notification-settings" id="friends-notification-settings">
		<thead>
			<tr>
				<th class="icon"></th>
				<th class="title"><?php _ex( 'Subscriptions', 'Friend settings on notification settings page', 'buddypress' ); ?></th>
		...
		<tbody>
			<tr id="friends-notification-settings-request">
				<td></td>
				<td><?php _ex( 'A member subscribes to your works', 'Friend settings on notification settings page', 'buddypress' ); ?></td>
			...
			<tr id="friends-notification-settings-accepted" style="display: none;">
				<td></td>
				<td><?php _ex( 'A member accepts your Subscription request', 'Friend settings on notification settings page', 'buddypress' ) ?></td>
             ...
             <tr id="friends-notification-settings-accepted">
				<td></td>
				<td><?php _ex( 'A member you subscribe to publishes or updates a work', 'Friend settings on notification settings page', 'buddypress' ) ?></td>
				<td class="yes"><input type="radio" name="notifications[notification_friends_new_custom]" id="notification-friends-friendship-accepted-yes" value="yes" <?php checked( $accept_requestss, 'yes', true ) ?>/><label for="notification-friends-friendship-accepted-yes" class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Yes, send email', 'buddypress' );
				?></label></td>
				<td class="no"><input type="radio" name="notifications[notification_friends_new_custom]" id="notification-friends-friendship-accepted-no" value="no" <?php checked( $accept_requestss, 'no', true ) ?>/><label for="notification-friends-friendship-accepted-no" class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'No, do not send email', 'buddypress' );
				?></label></td>
			</tr>
			 ...
			 <tr id="friends-notification-settings-accepted">
				<td></td>
				<td><?php _ex( 'A member comments on your work', 'Friend settings on notification settings page', 'buddypress' ) ?></td>
				<td class="yes"><input type="radio" name="notifications[notification_friends_new_post_comment]" id="notification-friends-friendship-accepted-yes" value="yes" <?php checked( $accept_requestss2, 'yes', true ) ?>/><label for="notification-friends-friendship-accepted-yes" class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Yes, send email', 'buddypress' );
				?></label></td>
				<td class="no"><input type="radio" name="notifications[notification_friends_new_post_comment]" id="notification-friends-friendship-accepted-no" value="no" <?php checked( $accept_requestss2, 'no', true ) ?>/><label for="notification-friends-friendship-accepted-no" class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'No, do not send email', 'buddypress' );
				?></label></td>
			</tr>
			...
			<tr id="friends-notification-settings-accepted">
				<td></td>
				<td><?php _ex( 'A member replies to your comment on a work', 'Friend settings on notification settings page', 'buddypress' ) ?></td>
				<td class="yes"><input type="radio" name="notifications[notification_friends_new_post_getcomment_reply]" id="notification-friends-friendship-accepted-yes" value="yes" <?php checked( $accept_requestss23, 'yes', true ) ?>/><label for="notification-friends-friendship-accepted-yes" class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Yes, send email', 'buddypress' );
				?></label></td>
				<td class="no"><input type="radio" name="notifications[notification_friends_new_post_getcomment_reply]" id="notification-friends-friendship-accepted-no" value="no" <?php checked( $accept_requestss23, 'no', true ) ?>/><label for="notification-friends-friendship-accepted-no" class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'No, do not send email', 'buddypress' );
				?></label></td>
			</tr>
